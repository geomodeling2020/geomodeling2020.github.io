...
let dataset = [];
/*d3.csv("./data/time_use_average.csv").row( 
  (d,i) => {
    return{
    state: d["GEO"],
    work: +d["work"],
  ...
  .get(
      ...
      if (rows.length>0) {
        ...
        console.log("First row: ",rows.length-1);
        dataset=rows;
    }
  }
  );*/

  d3.csv("data/time_use_average.csv", function(data){
    console.log(data);
  }, function(error, rows){
   dataset=rows; 
   /* console.log(rows); */
...
function selectedActivity(d) {
  switch (d) {
    ...
    case 'Average Household': return "act_hhold";
...
function actLabel(act){
  switch (act) {
    ...
    case 'act_pcare': return "personal_care";
    case 'act_educ': return "study";
    case 'act_hhold': return "household";
    case 'act_travel': return "travel";
  }
}
